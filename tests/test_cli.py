...
def test_run_without_out_prints_the_csv(capsys):
...
def test_run_reports_a_missing_input_file(tmp_path):
    assert main(["run", str(tmp_path / "nope.csv")]) == 1
...
def test_unwritable_out_is_a_runtime_error(tmp_path, capsys):
    out = tmp_path / "no-such-dir" / "o.csv"
    assert (
...
def test_check_accepts_a_lexicon_csv():
    from strands.cli import main
...
def test_word_save_writes_the_lint_accept_csv(tmp_path, capsys):
...
def test_word_save_defaults_to_a_timestamped_name(tmp_path, monkeypatch, capsys):
    ...
    assert re.fullmatch(r"\d{8}-\d{6}-indeagó\.csv", saved.name)
