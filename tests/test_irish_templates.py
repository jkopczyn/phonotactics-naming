...
def test_space_literal_splits_into_separate_words():
    words = build_construction(
        "ADJ", {"NAME": entry("mˠaːɾʲə", gender="f"), "ADJ": entry("bˠaːnˠ")}, IRISH, TABLE
    )
...
def test_len_if_f_does_not_lenite_after_a_masculine_noun():
    words = build_construction(
        "ADJ", {"NAME": entry("pˠaːd̪ˠɾˠəɟ", gender="m"), "ADJ": entry("ɾˠuə")}, IRISH, TABLE
    )
...
def test_patro_ni_lenites_the_genitive_father():
    words = build_construction(
        "PATRO_NI", {"FATHER": entry("kaːnˠ", declension="m1")}, IRISH, TABLE
    )
...
def test_patro_o_takes_the_genitive_without_lenition():
    words = build_construction("PATRO_O", {"FATHER": entry("kaːnˠ", declension="m1")}, IRISH, TABLE)
    assert ipa(words[0]) == "oːkaːnʲ"  # Ó Cáin


def test_compound_lenites_the_second_element():
    words = build_construction(
        "COMPOUND", {"FIRST": entry("l̪ˠasˠəɾʲ"), "SECOND": entry("kosˠ")}, IRISH, TABLE
    )
...
def test_of_masculine_genitive_article_lenites():
    words = build_construction(
        "OF",
        {"NAME": entry("ʃaːnˠ"), "NOUN": entry("bˠaːd̪ˠ", gender="m", declension="m1")},
        IRISH,
        TABLE,
    )
...
def test_article_blocks_lenition_before_a_coronal():
    words = build_construction(
        "OF",
        {"NAME": entry("ʃaːnˠ"), "NOUN": entry("tʲiː", gender="m", declension="d4")},
        IRISH,
        TABLE,
    )
...
def test_article_gives_s_the_t_prefix():
    words = build_construction(
        "OF",
        {"NAME": entry("ʃaːnˠ"), "NOUN": entry("sˠɔl̪ˠəsˠ", gender="m", declension="m1")},
        IRISH,
        TABLE,
    )
...
def test_article_feminine_genitive_is_na_with_h_prothesis():
    words = build_construction(
        "OF",
        {"NAME": entry("ʃaːnˠ"), "NOUN": entry("iːhə", gender="f", declension="d4")},
        IRISH,
        TABLE,
    )
...
def test_empty_gen_ipa_still_derives_the_regular_genitive():
    words = build_construction(
        "GEN", {"NAME": entry("mˠak", gen_ipa="", declension="m1")}, IRISH, TABLE
    )
    assert ipa(words[0]) == "mʲɪc"
