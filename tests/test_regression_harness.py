...
def test_mode_e_is_dutch_only():
    def both(t):
        return len(
            [r for r in read_attested(t) if r["source_ipa"].strip() and r["target_ipa"].strip()]
        )
...
def test_predicted_rows_are_dropped():
    assert not any(
        r["note"].startswith("PREDICTED-NOT-ATTESTED") for r in read_attested("arabic-egy")
    )
